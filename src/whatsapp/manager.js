...
// userId → { sock, cancelled }  — sockets that are pending QR scan
const pendingSockets = new Map();

// Global per-user flag — QR sent once, persists across silent reconnects
// Reset only on connect or explicit cancel
const qrAlreadySent = new Set();
...
async function cancelPendingConnection(userId) {
  ...
  qrAlreadySent.delete(String(userId)); // allow fresh QR next time
}
...
async function startPersonalSession(userId, onQR, onConnected, onDisconnected) {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    // ── QR received — send ONCE only (global flag per userId) ──────────
    if (qr && onQR && !qrAlreadySent.has(String(userId))) {
      qrAlreadySent.add(String(userId));
      try {
    ...
    if (connection === 'open') {
      ...
      pendingSockets.delete(userId);
      qrAlreadySent.delete(String(userId)); // reset for next time
...
// User gives their WA phone number → gets 8-digit code → enters in WhatsApp
async function startPairingSession(userId, phoneNumber, onCode, onConnected, onDisconnected) {
  ...
  const timeout = setTimeout(async () => {
    if (pendingSockets.has(userId)) {
      logger.info(`Pairing timeout for ${userId}`);
      await cancelPendingConnection(userId);
    }
  }, 5 * 60 * 1000);
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      logger.info(`WA paired & connected for ${userId} (${number})`);
      if (onConnected) await onConnected(number).catch(() => {});
    }
    ...
    if (connection === 'close') {
      ...
      if (loggedOut) {
        await clearSession(userId).catch(() => {});
        if (onDisconnected && !pending.cancelled) await onDisconnected('logged_out').catch(() => {});
      } else if (!pending.cancelled) {
        logger.info(`WA silent reconnect for ${userId} in 8s`);
        setTimeout(() => {
          if (!activeSockets.has(userId)) {
  ...
  // Small delay to let socket initialise
  ...
  try {
    ...
    logger.info(`Pairing code for ${userId}: ${formatted}`);
    if (onCode) await onCode(formatted);
  } catch (err) {
    logger.error(`requestPairingCode failed for ${userId}: ${err.message}`);
    await cancelPendingConnection(userId);
    throw err;
  }
}
